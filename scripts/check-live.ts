async function test() {
  const loginRes = await fetch('https://practicalroofems.online/api/v1/auth/login', {
    method: 'POST',
  ...
  const depRes = await (await fetch('https://practicalroofems.online/api/v1/departments', { headers: authHeaders })).json();
  ...
  const empRes = await (await fetch('https://practicalroofems.online/api/v1/employees?limit=50', { headers: authHeaders })).json();
  ...
  if (vacant) {
    ...
    await fetch('https://practicalroofems.online/api/v1/employees/' + vacant.id, {
      method: 'DELETE',
      headers: authHeaders
    });
  }

  const finalEmpRes = await (await fetch('https://practicalroofems.online/api/v1/employees?limit=50', { headers: authHeaders })).json();
