async function validateAllLogins() {
  ...
  console.log(`\nValidating live logins on https://practicalroofems.online for ${employees.length} accounts...\n`);

  let successCount = 0;
  for (const emp of employees) {
    try {
      const res = await fetch('https://practicalroofems.online/api/v1/auth/login', {
        method: 'POST',
